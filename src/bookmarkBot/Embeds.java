...
public class Embeds {
	public static void sendInfoEmbed(MessageChannel channel, User user) {
			...
			info.addField("`bbbookmark <link> <title>`", "Set a bookmark at this link.", false);
			info.addField("`bbbookmarks`", "Get a list of your bookmarks", false);
			info.addField("`bbdel <BookmarkID>`", "Delete the bookmark with this id.", false);
	...
	public static void sendBookmarkListEmbed(MessageChannel channel, User user) {
		if(BookmarkUtil.hasUser(user)) {
			...
			noBms.setDescription("Try using `bbhelp` to find out how to set one.");
